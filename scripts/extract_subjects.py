...
print((dataframe_from_csv(args.itemids_file)).index.values) 
...
if args.itemids_file:
    print(dataframe_from_csv(args.itemids_file)) 
    items_to_keep = set([int(itemid) for itemid in (dataframe_from_csv(args.itemids_file)).index.values])
    print ("balagoorio")
    print (items_to_keep)
else:
    items_to_keep = None
